...
In this task, we explore the behavior of the geometrically weighted out-degree (**gwodegree**) used in ERGMs for directed networks.
...
# IMPORTANT: You must call the variables to make them show up in the PDF
p1
p2
```
...
# 1. Load the data as a raw matrix
# We set header = FALSE because adjacency matrices often don't have column names
adj_m1 <- as.matrix(read.csv(file_path, header = FALSE))

# 3. Convert to network
# We specify directed = TRUE for a friendship network
glasgow1 <- as.network(adj_m1, directed = TRUE)

# 4. Estimate M1 (Edges only)
